...
function computeStats(trades: Trade[]) {
  ...
  const totalPnlCents = settled.reduce((s, t) => s + (t.pnlTotal ?? 0), 0);
  const avgEvCents =
    trades.length > 0
  ...
  let sharpeApprox = 0;
  if (pnls.length > 1) {
    const mean = totalPnlCents / pnls.length;
...
export default function LiveStatsCards() {
  const { data: allTrades, error, isLoading } = useSWR<Trade[]>(
    "trades-pnl",
    getTrades,
  ...
  if (isLoading) {
    return (
      <div>
        <p className="section-label">Live Strategy Overview</p>
  ...
  if (error) {
    return (
      <div className="panel flex items-center gap-2 text-loss">
        <AlertCircle size={16} />
        <span className="text-sm">Failed to load live stats: {error.message}</span>
      </div>
    );
  }
  ...
  if (liveTrades.length === 0) {
    return (
      <div>
        <p className="section-label">Live Strategy Overview</p>
        <div className="panel flex items-center justify-center py-8 text-muted text-sm">
          No live trades yet
        </div>
      </div>
    );
  ...
  const s = computeStats(liveTrades);
  const totalPnlColor =
